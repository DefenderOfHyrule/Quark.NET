...
public sealed class TcpCommandBlock : ICommandBlock
{
    ...
    public TcpCommandBlock(TcpInterface tcp)
    {
        _tcp = tcp;
        _in  = tcp.ReadBytes(BlockSize);
    }
    ...
    public void SendBuffer(byte[] buf, int length) => _tcp.WriteBytes(buf, length);
    public byte[] GetBuffer(int len)     => _tcp.ReadBytes(len) ?? Array.Empty<byte>();
    ...
    public void ResponseEnd() => _tcp.WriteBytes(_out);
